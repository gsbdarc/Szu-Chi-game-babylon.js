...
with sync_playwright() as pw:
    ...
    page.goto(BASE+'/researcher');page.locator('#preview-link').wait_for()
    ...
    touch=mobile.new_page();touch.goto(BASE+'/?SESSION_ID=QA-touch-'+str(time.time_ns()));touch.wait_for_function('window.buffet',timeout=90000)
    ...
    empty=browser.new_page(viewport={'width':1200,'height':800});empty.goto(BASE+'/?condition=no_screenshot&SESSION_ID=QA-empty-'+str(time.time_ns()));empty.wait_for_function('window.buffet',timeout=90000)
