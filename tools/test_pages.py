...
def site(url):
    ...
    with tempfile.TemporaryDirectory(prefix='buffet-pages-') as temporary:
        build(Path(temporary) / 'Szu-Chi-game-babylon.js')
        handler = functools.partial(QuietHandler, directory=temporary)
        server = ThreadingHTTPServer(('127.0.0.1', 0), handler)
        thread = threading.Thread(target=server.serve_forever, daemon=True)
        thread.start()
        try:
            yield f'http://127.0.0.1:{server.server_port}/Szu-Chi-game-babylon.js/'
        finally:
            server.shutdown()
            server.server_close()
            thread.join()
...
def check(base):
    ...
    with sync_playwright() as pw:
        ...
        def observe(page):
            ...
            page.on('response', lambda response: models.add(urlsplit(response.url).path) if response.url.endswith('.glb') and response.ok else None)
        ...
        assert len(models) == 15, f'Expected 15 food models, loaded {len(models)}'
        ...
        report.update(foodsLoaded=len(models), portionsRestored=2, screenshotsRestored=2,
