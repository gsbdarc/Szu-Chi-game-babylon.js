...
export class ResearchSession {
  static async open(onStatus) {
    ...
    r.db=await new Promise((resolve,reject)=>{const req=indexedDB.open(r.localOnly?'common-table-babylon-preview':'common-table-babylon',1);req.onupgradeneeded=()=>req.result.createObjectStore('sessions');req.onsuccess=()=>resolve(req.result);req.onerror=()=>reject(new Error('Browser storage is unavailable. Enable site storage and reload.'));});
  ...
  setPortions(portions){this.data.portions=portions.map(p=>({portionId:p.portionId,foodId:p.foodId,plateId:'main',addedAt:p.addedAt,position:{x:p.x,y:p.y,z:p.z},rotation:p.angle,tilt:p.tilt}));}
