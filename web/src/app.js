...
import {prepareChicken,ChickenMotion} from './chicken.js';
...
class BuffetGame {
  ...
  async init(){
    ...
    const hall=buildHall(this.scene);this.rig=new B.TransformNode('Participant plate rig',this.scene);this.rig.position.x=stationX(0);this.plate=makePlate(this.scene);this.plate.parent=this.rig;this.plate.position=V(0,.93,-.59);this.plate.setEnabled(false);
    const post=new B.FxaaPostProcess('Antialiasing',1,this.camera);
    this.engine.runRenderLoop(()=>{const dt=this.engine.getDeltaTime()/1000;for(const p of this.portions)p.motion?.step(dt);this.scene.render();});window.addEventListener('resize',()=>{this.engine.resize();this.fitCamera();});
  ...
  get busy(){return this.moving||this.finished||this.finishing||this.capturing||!this.started||this.dialog.open;}
  ...
  async moveView(plate=false,animate=true){
    ...
    const target=plate?pose.target:V(x,1.62,-1.24),look=plate?pose.look:V(x,.95,-.28);
  ...
  settlePortions(){for(const p of this.portions){p.animating=false;p.root.position.set(p.x,p.y,p.z);p.motion?.finish();}}
  createPortion(food,record=null,animate=true){
    if(animate)this.settlePortions();
  ...
  bind(){
    ...
    this.canvas.addEventListener('pointermove',event=>{if(!this.press||this.press.id!==event.pointerId)return;if(Math.hypot(event.clientX-this.press.x,event.clientY-this.press.y)>9)this.press.drag=true;if(this.press.drag&&this.press.meta?.station===this.station)this.canvas.style.cursor='grabbing';});
    this.canvas.addEventListener('pointerup',event=>{const press=this.press;this.press=null;this.canvas.style.cursor='';if(!press||press.id!==event.pointerId||this.busy)return;const point=this.planePoint(event),onPlate=point&&Math.hypot(point.x,point.z)<.145;
      if(press.meta?.station===this.station&&!this.plateView){if(press.drag?onPlate:this.pick(event)?.pickedMesh?.metadata?.station===this.station)this.add();}
      else if(press.meta?.portion&&this.plateView&&press.drag&&onPlate){const p=this.portions.find(p=>p.portionId===press.meta.portion);p.animating=false;this.repack(p);Object.assign(p,this.placement.fit(p.shape,point));p.root.position.copyFromFloats(p.x,p.y,p.z);p.motion?.place(this.placement.grid);this.placement.occupy(p);this.research.setPortions(this.portions);this.research.record('portion_moved',p.foodId,p.portionId);this.fitCamera();this.refresh();}
    });this.canvas.addEventListener('pointercancel',()=>{this.press=null;this.canvas.style.cursor='';});
  }
}
